...
class GenerateSegmentIndicesDewaterMask:
    ...
    def __call__(self, results):
        ...
        lq_path = [results['lq_paths'][v] for v in neighbor_list]
        gt_path = [results['gt_paths'][v] for v in neighbor_list]
        mask_path = [results['mask_paths'][v] for v in neighbor_list]
...
@PIPELINES.register_module()
class GenerateSegmentIndicesDewaterMaskFour:
    """Generate frame indices for a segment. It also performs temporal
    augmention with random interval.
    ...
    def __call__(self, results):
        ...
        gt_path = [results['gt_paths'][v] for v in neighbor_list]
        mask_path = [results['mask_paths'][v] for v in neighbor_list]
        refmask_path = [results['refmask_paths'][v] for v in neighbor_list]

        results['lq_path'] = lq_path
        results['gt_path'] = gt_path
        results['mask_path'] = mask_path
        results['refmask_path'] = refmask_path
        results['interval'] = interval
...
@PIPELINES.register_module()
class GenerateSegmentIndicesDewater:
    """Generate frame indices for a segment. It also performs temporal
    augmention with random interval.
