...
class TripletRandomCrop:
    ...
    def __call__(self, results):
        ...
        if not gt_is_list:
            results['gt'] = [results['gt']]
        mask_is_list = isinstance(results['mask'], list)
        if not mask_is_list:
...
class QuadRandomCrop:
    ...
    def __call__(self, results):
        ...
        if not mask_is_list:
            results['mask'] = [results['mask']]
        refmask_is_list = isinstance(results['refmask'], list)
        if not mask_is_list:
            results['refmask'] = [results['refmask']]
        ...
        results['refmask'] = [
            v[top//4:(top + lq_patch_size)//4, left//4:(left + lq_patch_size)//4, ...]
            for v in results['refmask']
        ]
        ...
        if not mask_is_list:
            results['mask'] = results['mask'][0]
        if not refmask_is_list:
            results['refmask'] = results['refmask'][0]
        return results
