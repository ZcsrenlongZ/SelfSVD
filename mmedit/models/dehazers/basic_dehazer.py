...
class ALignPSNR(nn.Module):
    def __init__(self):
        super().__init__()
        self.alignnet = PWCNet(load_pretrained=True, weights_path="./pretrained_dirs/pwcnet-network-default.pth")
        self.alignnet.eval()
        self.metric = psnr
    def forward(self, pred, gt, crop_border, convert_to):
        with torch.no_grad():
            self.alignnet.to(gt.device)
            pred = TF.gaussian_blur(pred, kernel_size=5)
            aligned_pred, mask, _ = self.alignnet(pred.to(gt.device), gt ,test=True)
        return self.metric(tensor2img(aligned_pred*mask), tensor2img(gt*mask), crop_border, convert_to=convert_to)

class ALignSSIM(nn.Module):
    def __init__(self):
        super().__init__()
        self.alignnet = PWCNet(load_pretrained=True, weights_path="./pretrained_dirs/pwcnet-network-default.pth")
        self.alignnet.eval()
        self.metric = ssim
    def forward(self, pred, gt, crop_border, convert_to=None):
        with torch.no_grad():
            self.alignnet.to(gt.device)
            pred = pred.to(gt.device)
            pred = TF.gaussian_blur(pred, kernel_size=5)
            aligned_pred, mask, _ = self.alignnet(pred, gt, test=True)
        return self.metric(tensor2img(aligned_pred*mask), tensor2img(gt*mask), crop_border, convert_to=convert_to)
...
align_psnr = ALignPSNR() 
align_ssim = ALignSSIM()
...
class BasicDehazer(BaseModel):
    ...
    """
    allowed_metrics = {'L1': l1_loss, 'PSNR': psnr, 'SSIM': ssim, 'NIQE':niqe, 'PI':pi, 'NRQM':nrqm,
                       'AlignPSNR':align_psnr, 'AlignSSIM':align_ssim}
        ...
        for metric in self.test_cfg.metrics:
            if output.ndim == 4:  # (n, c, h, w)
                ...
                for i in range(0, output.size(1)):
                    ...
                    # 算指标
                    elif metric in ('NIQE','PI', 'NRQM'):
                        iqa = self.allowed_metrics[metric].to(gt.device)
                        avg.append(iqa(output[:, i, :, 24:-24, 24:-24].to(gt.device)).item())
                    elif metric in ('AlignPSNR', 'AlignSSIM'):
                        avg.append(self.allowed_metrics[metric](
                            output[:, i, :, :, :], gt[:, i, :, :, :],
                            crop_border, convert_to=convert_to))
    ...
    def _save_image(output, meta, save_path, iteration):
        file_ext = 'png'  # to save storage, change it to png if enough disk
