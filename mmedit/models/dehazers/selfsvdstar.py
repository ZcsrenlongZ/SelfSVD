...
from mmedit.models.backbones.basic_backbones.selfsvd_net import SelfSVDNet

torch.autograd.set_detect_anomaly(True)

@MODELS.register_module()
class SelfSVDStar(BasicDehazer):
    def __init__(self,
                 generator,
                 pixel_loss,
                 discriminator=None,
        ...
        self.model_t = SelfSVDNet(mid_channels=64, num_blocks=60)
        stage1_model_path = "./work_dirs/selfsvd/selfsvd.pth"
        self.init_teacher_model(stage1_model_path)
        self.init_student_model(stage1_model_path)

    def init_teacher_model(self,stage1_model_path):
        pretrained_dict = torch.load(stage1_model_path)['state_dict']
        new_state_dict = OrderedDict()
        for k, v in pretrained_dict.items():
            if k.split('.')[0] == 'generator':
                name = k[10:]
                new_state_dict[name] = v
        self.model_t.load_state_dict(new_state_dict, strict=True)
        self.model_t.eval()
        # Frozen teacher model
        for param in self.model_t.parameters():
            param.requires_grad = False        

    def init_student_model(self,stage1_model_path):
        pretrained_dict = torch.load(stage1_model_path)['state_dict']
        new_state_dict = OrderedDict()
        for k, v in pretrained_dict.items():
            if k.split('.')[0] == 'generator':
    ...
    def train_step(self, data_batch, optimizer):
        ...
        with torch.no_grad():
            ref_mask = torch.zeros((b, 1, 1, h//4, w//4)).to(gt.device)
            t_output = self._get_output_from_dict(self.model_t({"lq":gt[:, 0:1, ...],
                                                                "lq_ref":gt[:, 0:1, ...],
                                                                "refmask":ref_mask,
                                                                "is_train":False})).detach().repeat(1, t, 1, 1, 1)
            gt = t_output.clamp(0., 1.)
